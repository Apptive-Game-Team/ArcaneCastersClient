...
| `Background/background.png` | 클라이언트 PR (이슈 #54) | 1254x1254 RGB 불투명, 타일 가능. `PopupBookGround` 의 `ground.mat` `_MainTex` 이자 9개 씬의 UI 배경으로 같이 쓴다. 옛 아트는 1800x1000 비정방형이라 정방형 Plane 에 1.8:1 로 눌렸다. 1차 재생성본은 패치 경계가 뭉개지고 낱장 잎 액센트가 오브젝트처럼 읽혀 반려했고, 경계를 또렷하게 자르고 잎을 빼고 풀 다발만 작게 남긴 2차 후보 A 로 교체했다. 값 폭을 좁게 유지해 위에 올라가는 유닛과 UI 텍스트가 읽힌다 |
