...
| `Background/background.png` | 클라이언트 PR (이슈 #54) | 1254x1254 RGB 불투명. `PopupBookGround` 의 `ground.mat` `_MainTex` 로 쓰는 필드 바닥. 옛 아트는 1800x1000 비정방형이라 정방형 Plane 에 1.8:1 로 눌려 들어갔다. 정방형으로 다시 뽑아 그 왜곡을 없앴다. 값 폭을 좁게 유지해 위에 올라가는 유닛과 이펙트가 읽히게 했다. **남은 문제: 낱장 잎 액센트가 40 유닛 평면 기준으로 크다. 2x2 타일링으로 줄일 수 있으나 텍스처가 완전한 seamless 가 아니라 가운데 이음매가 보여 1x 로 두었다** |
